...
impl CredentialStore {
    ...
    pub fn store(key: &str, value: &str) -> DepotResult<()> {
        let entry = Entry::new(KEYRING_SERVICE, key)
            .map_err(|e| DepotError::Package(format!("Failed to create keyring entry: {}", e)))?;

        entry.set_password(value).map_err(|e| {
            DepotError::Package(format!("Failed to store credential in keychain: {}", e))
        })?;
    ...
    pub fn retrieve(key: &str) -> DepotResult<String> {
        let entry = Entry::new(KEYRING_SERVICE, key)
            .map_err(|e| DepotError::Package(format!("Failed to create keyring entry: {}", e)))?;
    ...
    pub fn delete(key: &str) -> DepotResult<()> {
        let entry = Entry::new(KEYRING_SERVICE, key)
            .map_err(|e| DepotError::Package(format!("Failed to create keyring entry: {}", e)))?;

        entry.delete_credential().map_err(|e| {
            DepotError::Package(format!("Failed to delete credential from keychain: {}", e))
        })?;
    ...
    #[cfg(unix)]
    pub fn set_secure_permissions(path: &Path) -> DepotResult<()> {
        use std::fs;
        use std::os::unix::fs::PermissionsExt;

        let mut perms = fs::metadata(path)
            .map_err(|e| DepotError::Package(format!("Failed to get file metadata: {}", e)))?
            .permissions();

        perms.set_mode(0o600); // rw------- (owner read/write only)
        fs::set_permissions(path, perms)
            .map_err(|e| DepotError::Package(format!("Failed to set file permissions: {}", e)))?;
    ...
    #[cfg(windows)]
    pub fn set_secure_permissions(_path: &Path) -> DepotResult<()> {
