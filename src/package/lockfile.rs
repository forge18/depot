...
impl Lockfile {
    /// Load lockfile from a directory (with automatic migration from package.lock to lpm.lock)
    pub fn load(dir: &Path) -> LpmResult<Option<Self>> {
        ...
        if new_path.exists() {
            ...
            let lockfile: Lockfile = serde_yaml::from_str(&content).map_err(|e| {
                LpmError::Package(format!("Failed to parse {}: {}", LOCKFILE_NAME, e))
            })?;
        ...
        if old_path.exists() {
            ...
            let lockfile: Lockfile = serde_yaml::from_str(&content).map_err(|e| {
                LpmError::Package(format!("Failed to parse {}: {}", OLD_LOCKFILE_NAME, e))
            })?;
    ...
    pub fn save(&self, dir: &Path) -> LpmResult<()> {
        ...
        let yaml = serde_yaml::to_string(self).map_err(|e| {
            LpmError::Package(format!("Failed to serialize {}: {}", LOCKFILE_NAME, e))
        })?;
