...
2. **Email** (if GitHub is not accessible):
   ...
   - Subject: "Security Vulnerability in Depot"
...
1. **Always use lockfiles**: Commit `depot.lock` to ensure reproducible builds
2. **Run security audits**: Use `depot audit` to check for known vulnerabilities
3. **Keep dependencies updated**: Regularly update packages with `depot update`
4. **Verify checksums**: Depot automatically verifies package checksums
5. **Review dependencies**: Be cautious when adding new dependencies

## Security Features

Depot includes several security features:
...
- **No postinstall scripts**: Depot does not execute arbitrary code during installation
...
- Depot does not execute postinstall scripts for security reasons
...
For more information about Depot's security features, see [docs/user/Security.md](docs/user/Security.md).
